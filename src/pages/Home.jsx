...
export default function Home() {
  ...
  return (
    <div className="page">
      <div style={{ background:'linear-gradient(160deg, #0A0A12 0%, #0D1A12 100%)', padding:'48px 24px 32px', borderBottom:'1px solid var(--border)', position:'relative', overflow:'hidden' }}>
        <div style={{ position:'absolute', top:-40, right:-40, width:180, height:180, borderRadius:'50%', background:'radial-gradient(circle, rgba(0,230,118,0.08) 0%, transparent 70%)', pointerEvents:'none' }} />
        <div style={{ maxWidth:480, margin:'0 auto' }}>
          ...
          <h1 style={{ lineHeight:1, marginBottom:4 }}>
            <span style={{ color:'var(--green)' }}>CHAU</span><span style={{ color:'var(--text)' }}>BONDI</span>
          </h1>
          <p style={{ color:'var(--text2)', fontSize:'0.9rem' }}>Compartí el viaje al Campus. Dividí el gasto.</p>

          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginTop:24 }}>
            <button className="btn btn-primary" onClick={() => navigate('/search?tipo=IDA')}
              style={{ flexDirection:'column', gap:4, padding:'18px 12px', height:'auto' }}>
              <span style={{ fontSize:'1.8rem' }}>🎓</span>
              <span style={{ fontSize:'0.95rem' }}>Ir al Campus</span>
            </button>
            <button className="btn btn-secondary" onClick={() => navigate('/search?tipo=VUELTA')}
              style={{ flexDirection:'column', gap:4, padding:'18px 12px', height:'auto' }}>
              <span style={{ fontSize:'1.8rem' }}>🏠</span>
          ...
          <button className="btn btn-secondary btn-full" onClick={() => navigate('/create')} style={{ marginTop:12 }}>
            ➕ Publicar mi viaje
          </button>
      ...
      <div className="container" style={{ paddingTop:24 }}>
        {pending.length > 0 && (
          <div className="alert alert-info" style={{ marginBottom:20 }}>
            ...
            <button className="btn btn-sm" onClick={() => navigate('/profile?tab=ratings')}
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
          ...
          <button onClick={() => navigate('/search')} style={{ background:'none', border:'none', color:'var(--green)', fontSize:'0.85rem', fontWeight:600, cursor:'pointer', padding:0 }}>
            Ver todos →
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <h3 style={{ fontFamily:'var(--font-head)', fontSize:'1.4rem' }}>SIN VIAJES AÚN</h3>
            <p>¡Publicá el primero y animá a tus compañeros a sumarse!</p>
            <button className="btn btn-primary" onClick={() => navigate('/create')} style={{ marginTop:20 }}>Publicar viaje</button>
          </div>
        ) : (
