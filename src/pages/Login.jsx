...
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    try { const data = await api.auth.login(form); login(data.token, data.user); navigate('/', { replace: true }); }
    ...
    <div style={{ minHeight:'100dvh', display:'flex', flexDirection:'column', background:'var(--bg)' }}>
      ...
      <div style={{ flex:1, padding:'32px 24px', maxWidth:480, margin:'0 auto', width:'100%' }}>
        ...
        <div style={{ marginTop:28, textAlign:'center', color:'var(--text2)', fontSize:'0.9rem' }}>
          ¿No tenés cuenta? <Link to="/register" style={{ fontWeight:600 }}>Registrate</Link>
        </div>
      </div>
    </div>
  );
